...
export default function DashboardScreen() {
  ...
  const { status, sensors, alarmEnabled, alarmTriggered, publish, toggleAlarm, dismissAlarm, config, logs } =
    useMqtt();
  ...
  const handleToggleLed = () => {
    ...
    publish(topic("led"), sensors.ledState ? "OFF" : "ON");
  };
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        >
          <View style={styles.controlLeft}>
            ...
            <View>
              ...
              <Text style={[styles.controlSub, { color: colors.mutedForeground }]}>
                {alarmTriggered ? "TRIGGERED!" : alarmEnabled ? `Armed (${topic("alarm")})` : "Disarmed"}
              </Text>
            </View>
          </View>
          <Switch
        ...
        <Text style={[styles.sectionLabel, { color: colors.mutedForeground }]}>MQTT LOGS</Text>
        <View style={[styles.controlCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={{ width: "100%" }}>
            {logs.length === 0 ? (
              <Text style={[styles.controlSub, { color: colors.mutedForeground }]}>No traffic yet</Text>
            ) : (
              logs
                .slice(-6)
                .reverse()
                .map((entry) => (
                  <Text key={entry.id} style={[styles.controlSub, { color: colors.mutedForeground }]} numberOfLines={1}>
